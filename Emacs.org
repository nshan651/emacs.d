...
Include this for improved start-times.
...
Add some global variable settings. 
...
This section configures UI elements to be more minimal and sleek.
...
I use ==modue-vivendi== as my main theme.
...
This config uses =evil-mode= for VI-like modal editing. =general.el= is used for easy keybinding configuration that integrates well with =which-key=
...
I use several packages such as =ivy=, =which-key=, and =counsel= to extend Emacs' UI capabilities.
...
Ivy is a completion framework for Emacs. It provides a minimal yet powerful selection menu that appears when you open files, switch buffers, and for many other tasks in Emacs. Counsel is a customized set of commands to replace =find-file= with =counsel-find-file=, etc which provide useful commands for each of the default completion commands.
...
Prescient provides some helpful behavior for sorting Ivy completion candidates based on how recently or frequently you select them.
...
Uses visual-fill-column to center org-mode buffers for a more pleasing writing experience as it centers the contents of the buffer horizontally to seem more like you are editing a document. 
...
=lsp-mode= enables IDE-like functionality for many different programming languages via “language servers” that speak the Language Server Protocol. Check the [[https://emacs-lsp.github.io/lsp-mode/page/languages/][documentation]] for adding a language.
...
=lsp-treemacs= provides nice tree views for different aspects of your code like symbols in a file, references of a symbol, or diagnostic messages that are found in your code.
...
=dap-mode= is an excellent package for bringing rich debugging capabilities to Emacs via the Debug Adapter Protocol. Documentation can be found [[https://github.com/daviwil/emacs-from-scratch/blob/master/Emacs.org][here]].
...
[[https://github.com/bbatsov/projectile][Projectile]] is a project management library for Emacs which makes it a lot easier to navigate around code projects for various languages.
...
Emacs’ built in commenting functionality doesn’t always comment things in the way you might expect so we use [[https://github.com/redguardtoo/evil-nerd-commenter][evil-nerd-commenter]] to provide a more familiar behavior.
...
[[https://github.com/Fanael/rainbow-delimiters][rainbow-delimiters]] is a useful programming mode because it colorizes nested parentheses and brackets according to their nesting depth.
...
- =C-c C-p= / =C-c C-n= - go back and forward in the buffer’s prompts (also brackets with evil-mode)
...
The =eterm-256color= package enhances the output of term-mode to enable handling of a wider range of color codes so that many popular terminal applications look as you would expect them to. 
