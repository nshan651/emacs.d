...
** Message buffer
Increase the number of lines in the =*Messages*= buffer.
#+begin_src emacs-lisp :tangle "early-init.el"
...
** Indent Bars
=indent-bars= shows thin vertical guides at each indentation level.
#+begin_src emacs-lisp :tangle "lisp/ui-config.el" :mkdirp yes
...
** Font Faces
The =ns/org-font-setup= function configures various text faces to tweak the sizes of headings and use variable width fonts in most cases so that it looks more like we're editing a document in =org-mode=.  We switch back to fixed width (monospace) fonts for code blocks and tables so that they display correctly.
...
** Project
I use the built-in =project.el= for project management. I switched away from [[https://projectile.mx/][Projectile]] after realizing that I was only using a small subset of its available features.
#+begin_src emacs-lisp :tangle "lisp/development-config.el" :mkdirp yes
  (defun ns/ripgrep ()
...
#+end_src
** Whisper
[[https://github.com/natrys/whisper.el][Whisper]] is a speech-to-text interface for emacs using OpenAI's speech recognition model.
#+begin_src emacs-lisp :tangle "lisp/applications-config.el" :mkdirp yes
  (use-package whisper
...
#+end_src
** Focus
Based on [[https://github.com/LouisKottmann/ns.el/tree/master][zygospore]], focuses the current window to the full frame. Perspective-aware.
#+begin_src emacs-lisp :tangle "lisp/window-config.el" :mkdirp yes
