...
This snippet adds a hook to =org-mode= buffers so that =efs/org-babel-tangle-config= gets executed each time such a buffer gets saved.  This function checks to see if the file being saved is the Emacs.org file you're looking at right now, and if so, automatically exports the configuration here to the associated output files. Currently, this is disabled.
...
#+begin_src emacs-lisp :tangle "lisp/development.el" :mkdirp yes
...
#+RESULTS:
| evil-collection-python-set-evil-shift-width | lsp-deferred |
